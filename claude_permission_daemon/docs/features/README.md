...
* Your implementation plan must be written to the feature markdown file and committed to git before beginning implementation.
...
* At the end of every Milestone and Feature you must (in order):
  ...
  2. Run all tests and ensure that ALL tests are passing. You MAY NOT consider a Milestone complete until ALL tests that were passing at the beginning of the Milestone are still passing, unless given explicit human approval to defer testing until later.
...
* Every feature must end with an "Acceptance Criteria" Milestone. This Milestone must include tasks to:
  ...
  3. ALL test sessions must be passing successfully.
